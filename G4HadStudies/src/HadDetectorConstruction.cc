...
G4VPhysicalVolume* HadDetectorConstruction::Construct()
{
 ...
 // G4double worldZ  = targetZ + cm;
  ...
  // World
  //
   G4double world_hx = pos_x + cm;
   G4double world_hy = pos_y + cm;
   G4double world_hz = pos_z + cm;

  G4Box* solidW = new G4Box("World", world_hx, world_hy, world_hz);
  ...
  G4Box* solidT = new G4Box("Target", pos_x, pos_y, pos_z);
  ...
  vDCinfo.push_back(2.*pos_z);
  vDCinfo.push_back(2.*pos_y);
  vDCinfo.push_back(2.*pos_x);
...
void HadDetectorConstruction::SetTargetWidth(G4double val)  
{
  if(val > 0.0) {
    pos_x = val;
...
void HadDetectorConstruction::SetTargetHeight(G4double val)  
{
  if(val > 0.0) {
    pos_y = val;
    G4RunManager::GetRunManager()->GeometryHasBeenModified();
