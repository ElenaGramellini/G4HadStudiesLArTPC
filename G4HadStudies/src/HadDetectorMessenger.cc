...
:Detector(Det)
{
  ...
  rCmd->SetGuidance("Set width of the target");
  rCmd->SetParameterName("pos_x",false);
  rCmd->SetUnitCategory("Length");
  rCmd->SetRange("pos_x>0");
  rCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  lCmd = new G4UIcmdWithADoubleAndUnit("/testhadr/TargetHeight",this);
  lCmd->SetGuidance("Set height of the target");
  lCmd->SetParameterName("pos_y",false);
  lCmd->SetUnitCategory("Length");
  lCmd->SetRange("pos_y>0");
  lCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  pCmd = new G4UIcmdWithADoubleAndUnit("/testhadr/TargetLength",this);
  pCmd->SetGuidance("Set length of the target");
  pCmd->SetParameterName("pos_z",false);
  pCmd->SetUnitCategory("Length");
  pCmd->SetRange("pos_z>0");
  pCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
void HadDetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  //else if( command == rCmd ) 
    ...
    Detector->SetTargetHeight(lCmd->GetNewDoubleValue(newValue));
  else if( command == pCmd ) 
    Detector->SetTargetLength(pCmd->GetNewDoubleValue(newValue));
